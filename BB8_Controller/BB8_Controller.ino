...
/****************** User Config *************************/
// declare motor, servo, and audio ints
int x1, x2, y1, y2, cw, ccw, aud1;
/**********************************************************/
...
void setup() {        // Setup only runs once!
  ...
  // Open a writing pipe on each radio
...
void loop() {
  ...
  message[4] = aud1;                     // AUD1(D2) (Audio)                      [X2, Y2, X1, Y1, 00-(aud1)]
  message[4] = (message[4] << 3) | cw;  // CW (Clockwise Bit)                   [X2, Y2, X1, Y1, 0000    00(cw)]
